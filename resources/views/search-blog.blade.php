...
<div id="fullpage">
  ...
  <div id="content">
      <div class="col-lg-8 offset-lg-2">
        <h3 class="title-list display-4 text-center">List Post</h3>
        <u class="result">Tìm thấy <b>{{$posts->count()}}</b> kết quả cho từ khoả "<b>{{$k}}</b>"</u>
        <div class="list-post">
            @if ($posts->count() >0)
                @foreach ($posts as $p)
                <div class="one-post">
                    <div class="row">
                        <div class="col-lg-5">
                            <div class="thum-post">
                                <a href="/post/{{$p->slug}}">
                                    <img src="{{asset('/storage/img/'.$p->thumbnail)}}" class="img-fluid" alt="">
                                </a>
                            </div>
                        </div>
                       
                        <div class="col-lg-7">
                            <h3 class="tit-ar"><a href="/post/{{$p->slug}}">{{$p->title}}</a></h3>
                            <p class="m-0">{{ Str::limit($p->describe, 100) }}</p>
                            <a href="/post/{{$p->slug}}" class="btn btn-secondary btn-sm rounded-0 text-white mt-3">Read More</a>
                        </div>
                    </div>
                </div>
                {{-- end one post --}}
